...
export async function getById(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (isNaN(id)) {
      throw createError(400, 'Invalid portfolio ID')
    }
...
export async function getByVendor(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (isNaN(vendorId)) {
      throw createError(400, 'Invalid vendor ID')
    }
...
export async function create(req: Request, res: Response, next: NextFunction) {
  try {
    const { id_vendor, id_package, id_category, media_url, description, location, label, sort_order } = req.body
    if (!id_vendor || !media_url) {
      throw createError(400, 'id_vendor and media_url are required')
    }
...
export async function update(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const { id_package, id_category, media_url, description, location, label, sort_order, status } = req.body
...
export async function reorder(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!Array.isArray(items)) {
      throw createError(400, 'items must be an array of { id_portfolio: number, sort_order: number }')
    }
