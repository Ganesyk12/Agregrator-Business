...
async function fetchRequests() {
  try {
    ...
    requests.value = (json.data || []).filter((r: PaymentRequest) => r.status === 'approved')
  } catch (err) {
...
function openDetail(r: PaymentRequest) {
  router.push(`/rfp-payments/${r.id_request}`)
}
...
const filtered = computed(() => {
  ...
  result = [...result].sort((a: any, b: any) => {
    ...
    if (col === 'total_amount') {
      ...
      va = String(a.outstanding || 0)
      vb = String(b.outstanding || 0)
...
function outstandingClass(r: PaymentRequest) {
  if (r.outstanding <= 0) return 'color: #5cb85c;'
  return 'color: #d9534f;'
}
</script>
        ...
        <table class="table table-striped table-bordered">
          <thead>
            <tr>
              ...
              <th @click="setSort('outstanding')" style="cursor: pointer; user-select: none;">Outstanding <i v-if="sortColumn === 'outstanding'" :class="sortDirection === 'asc' ? 'fa fa-sort-asc' : 'fa fa-sort-desc'"></i><i v-else class="fa fa-sort" style="color: #ccc;"></i></th>
          ...
          <tbody>
            <tr v-for="r in paginated" :key="r.id_request">
              ...
              <td :style="outstandingClass(r)"><strong>{{ formatCurrency(r.outstanding) }}</strong></td>
              <td>{{ r.requested_by || '-' }}</td>
              <td>{{ new Date(r.request_date).toLocaleDateString('id-ID') }}</td>
              <td style="white-space: nowrap;">
                <button class="btn btn-primary btn-xs" @click="openDetail(r)" title="Process Payment"><i class="fa fa-credit-card"></i></button>
              </td>
            </tr>
            <tr v-if="paginated.length === 0">
              <td colspan="8" style="text-align: center;">No approved requests pending payment.</td>
            </tr>
          </tbody>
        </table>
      </div>
