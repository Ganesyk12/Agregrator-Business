...
watch(() => props.visible, async (val) => {
  if (val) {
    ...
    if (props.mode === 'add') {
      form.value = { email: '', role_code: '' }
    } else if (props.userRole) {
      form.value = {
        email: props.userRole.email,
        role_code: props.userRole.role_code,
      }
    }
  }
})
...
function save() {
  if (!form.value.email || !form.value.role_code) return
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
              ...
              <div class="row">
                <div class="col-md-6">
                  ...
                  <div class="form-group" style="text-align: left;">
                    ...
                    <select v-model="form.role_code" class="form-control">
                      <option value="" disabled>Select role</option>
                      <option v-for="r in roles" :key="r.role_code" :value="r.role_code">
                        {{ r.name }} ({{ r.role_code }})
                      </option>
                    </select>
                  </div>
        ...
        <div class="modal-footer">
          ...
          <button v-if="mode !== 'detail'" type="button" class="btn btn-primary" @click="save" :disabled="!form.email || !form.role_code">
            Save changes
          </button>
        </div>
      </div>
